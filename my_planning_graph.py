...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if node_a.prenodes <= self.s_levels[level]: 
                # print("Action ", action.name, " is added")
                #set parents
                node_a.parents = node_a.prenodes
                # set child node for parents
                for parent in node_a.parents:
                    parent.children.add(node_a)
                # add node in a_levels
    ...
    def add_literal_level(self, level):
        ...
        for node in self.a_levels[level - 1]:
            # add all effnodes as a literal node
            for effnode in node.effnodes:
                # set parents
                effnode.parents.add(node)
                # set child node for parents
                node.children.add(effnode)
                # add node in s_levels
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # if any negation intersection between nodes' effects
        if len(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)) > 0 or len(set(node_a1.action.effect_rem) & set(node_a2.action.effect_add)) > 0:
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # if any negation intersection between one node's precondition and the other's effect
        if len(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)) > 0 or len(set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)) > 0 or len(set(node_a1.action.precond_pos) & set(node_a2.action.effect_rem)) > 0 or len(set(node_a1.action.precond_neg) & set(node_a2.action.effect_add)) > 0:
            return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for prenode_a1 in node_a1.parents:
            for prenode_a2 in node_a2.parents:
                # if any precondition pair is mutex
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent_s1 in node_s1.parents:
            for parent_s2 in node_s2.parents:
                # if any precondition pair is not mutex, two nodes are not mutex
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            for s_level in self.s_levels:
                # if any literal from goal exists in this level, add cost 
                if g in [t.symbol for t in s_level if t.is_pos]:
                    level_sum += level
                    break
                level += 1
